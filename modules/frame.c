...
void render_frame(view_t *view, render_buf_t *out) {
	frame_data_t *data = view->data;
	SHORT inner_w, inner_h, start_x, start_y;
...
BOOLEAN frame_view_destroyv(view_t *frame, view_t *child) {
	...
	if((removed = frame_view_removev(frame, child)) == TRUE) {
		destroy_view(child);
	}
